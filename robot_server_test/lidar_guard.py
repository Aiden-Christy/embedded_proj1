        ...
        while self._running:
            try:
                ...
                for measurement in scan_generator:
                    ...
                    # ── DEBUG ──────────────────────────────────────────────
                    # Prints every reading so you can see what angle
                    # "forward" is on your robot. Remove once confirmed.
                    print(f"[LIDAR] angle={measurement.angle:6.1f}°  dist={measurement.distance:6.0f}mm")
                    # ───────────────────────────────────────────────────────
                    ...
                    with self._lock:
                        # Update blocked state for every in-arc point.
                        # A close reading sets it True; a far reading clears it.
                        # Out-of-arc readings don't change the state so a
                        # momentary clear from a far-away angle won't mask
                        # a real obstacle.
                        if in_arc:
                            self._blocked = close
